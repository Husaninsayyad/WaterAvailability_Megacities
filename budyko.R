...
plot_data <- plot_data %>%
  filter(city %in% water_trends$city & water_trends$p_value[match(city, water_trends$city)] < 0.05)
...
BUDYKO_SIG <- BUDYKO +
  geom_text_repel(
    ...
    point.padding = 0.15,   # space from the arrow tip
...
final_plot <- ggdraw() +
  draw_plot(BUDYKO_SIG) +
  draw_plot(inset_plot, x = 0.07, y = 0.65, width = 0.15, height = 0.4)
final_plot

# Save
ggsave("F5_gleammswep_Budyko_Two_Periods_sig.png", plot = final_plot, dpi = 300, width = 16, height = 10, bg = "white")
