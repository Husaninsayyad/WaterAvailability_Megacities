...
w_trend_cross_source <- function(df_p, df_et, label) {
  left_join(
    df_p %>% filter(variable == "tp") %>% 
      select(city, country, lon, lat, year, P = value),
    df_et %>% filter(variable == "e")  %>% 
...
data_mswep <- combined_source_data %>% 
  filter(data_source == "mswep/gleam" & variable == "tp")
data_gleam <- combined_source_data %>%
  filter(data_source == "mswep/gleam" & variable == "e")

trend_terra <- data_terra %>% group_by(city, country, lon, lat, variable) %>% 
  summarise(Trend = sens.slope(value)$estimates, .groups = "drop")
trend_era   <- data_era   %>% group_by(city, country, lon, lat, variable) %>% 
  summarise(Trend = sens.slope(value)$estimates, .groups = "drop")
trend_mswep <- data_mswep %>% group_by(city, country, lon, lat, variable) %>% 
  summarise(Trend = sens.slope(value)$estimates, .groups = "drop")
trend_gleam <- data_gleam %>% group_by(city, country, lon, lat, variable) %>% 
  summarise(Trend = sens.slope(value)$estimates, .groups = "drop")
...
terra_w       <- w_trend_same_source(data_terra, "TERRA W")  %>%
  transmute(city, country, `TERRA W`       = Trend)
era_w         <- w_trend_same_source(data_era,   "ERA5L W")  %>% 
  transmute(city, country, `ERA5L W`       = Trend)
mswep_gleam_w<- w_trend_cross_source(data_mswep, data_gleam,"mswep/GLEAM W")%>% 
  transmute(city, country, `mswep/GLEAM W` = Trend)
...
tick_labs <- c(
  ...
  "TERRA AET" = "TERRACLIMATE", "ERA5L AET" = "ERA5-Land",
  "GLEAM AET" = "GLEAM-v4-1a",
  "TERRA W" = "TERRACLIMATE", "ERA5L W" = "ERA5-LAND", 
...
BOXPLOT <- ggplot(long_df, aes(x = Dataset, y = Trend, fill = Quantity)) +
  geom_boxplot(outlier.shape = 21, outlier.size = 1.5,
               alpha = 0.95, linewidth = 0.3, width = 0.5) +
  scale_fill_manual(values = qty_cols, 
    name = NULL, breaks = c("P", "AET", "W"), labels = c("P", "AET", "W")) +
...
ggsave("outputs/F4_Boxplot.png",
       plot = BOXPLOT, dpi = 600, width = 16, height = 10, bg = "white")
