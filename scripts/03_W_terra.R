# 03_W_terra.R
# Water availability trend map - TerraClimate
...
# SP = significant positive, NSP = non-significant positive, SN = significant negative, NSN = non-significant negative
map_data <- map_data %>%
  mutate(
...
label_lon <- function(x) ifelse(x == 0, "0°", paste0(abs(x), "°", ifelse(x < 0, "W", "E")))
label_lat <- function(y) ifelse(y == 0, "0°", paste0(abs(y), "°", ifelse(y < 0, "S", "N")))
...
make_key_grob <- function(triangle_fill = "white", show_circle = FALSE, show_star = FALSE,
...
key_SP  <- make_key_grob(show_circle = TRUE,  show_star = TRUE,  label_text = "SP",  tri_pch = 24)
key_NSP <- make_key_grob(show_circle = FALSE, show_star = FALSE, label_text = "NSP", tri_pch = 24)
key_SN  <- make_key_grob(show_circle = TRUE,  show_star = TRUE,  label_text = "SN",  tri_pch = 25)
key_NSN <- make_key_grob(show_circle = FALSE, show_star = FALSE, label_text = "NSN", tri_pch = 25)
...
legend_row <- plot_grid(
  left_legend_draw, ggdraw() + theme_void(), right_legend_draw,
  ncol = 3, rel_widths = c(0.22, 0.03, 0.75), align = "h"
)

final_plot <- plot_grid(ggdraw(main_map), legend_row, ncol = 1, rel_heights = c(1, 0.06))

ggsave("outputs/F1_terra_Water_Availability_Trend1.png", final_plot, width = 16, height = 10, dpi = 800, bg = "white")
