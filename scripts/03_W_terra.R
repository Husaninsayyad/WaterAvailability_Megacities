...
world_bb <- ne_download(category = "physical", 
                        type = "wgs84_bounding_box", returnclass = "sf")
sf::sf_use_s2(TRUE)
...
make_key_grob <- function(triangle_fill = "white", show_circle = FALSE,
  ...
  txt <- textGrob(label_text, x = unit(0.30, "npc"), y = y,
                  just = "left", gp = gpar(fontsize = 12))
  grob_list <- c(grob_list, list(txt))
  grobTree(children = do.call(gList, grob_list))
}

key_SP  <- make_key_grob(show_circle = TRUE, 
                         show_star = TRUE,  label_text = "SP",  tri_pch = 24)
key_NSP <- make_key_grob(show_circle = FALSE,
                         show_star = FALSE, label_text = "NSP", tri_pch = 24)
key_SN  <- make_key_grob(show_circle = TRUE, 
                         show_star = TRUE,  label_text = "SN",  tri_pch = 25)
key_NSN <- make_key_grob(show_circle = FALSE, 
                         show_star = FALSE, label_text = "NSN", tri_pch = 25)
...
left_legend_draw  <- ggdraw() + draw_grob(legend_with_title_custom, 
                      x = 0.05, y = 0.5, width = 1, height = 1)
right_legend_draw <- ggdraw() + draw_grob(fill_legend_grob, 
                     x = 0.97, y = 0.5, width = 1, height = 1, hjust = 1)
...
final_plot <- plot_grid(ggdraw(main_map), legend_row, ncol = 1,
                        rel_heights = c(1, 0.06))

ggsave("outputs/F1_terra_Water_Availability_Trend1.png", 
