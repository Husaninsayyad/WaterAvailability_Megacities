...
# Cities as sf points
spatial_pts <- st_as_sf(terra_tp, coords = c("lon", "lat"), crs = 4326)
...
summary(continent_data$Trend)

# Ascending cutpoints; min_val and max_val are dataset bounds
trend_breaks_asc <- c(min_val, -5, -3, -1, -0.5, 0, 0.5, 1, 3, 5, max_val)

# Labels for each interval (must be length = length(breaks)-1)
trend_labels_asc <- c(
  paste0(format(min_val, nsmall = 2), " – -5.00"),
  "-05.00 – -3.00",
  "-3.00 – -1.00",
  "-1.00 – -0.50",
  "0.00 – -0.50",
  ...
  "1.00 – 3.00",
  "3.00 – 5.00",
  paste0("5.00 – ", format(max_val, nsmall = 2))
)

# Legend order (positive first, then negative)
trend_labels_pos_first <- rev(trend_labels_asc)

# Colors (must also match the number of intervals = 11 here)
trend_colors <- c(
  "darkblue",
  "blue3",
  "blue",
...
create_continent_plot <- function(data, world_map, continent_name, bounds) {
  ggplot() +
    ...
    geom_point(
      ...
      aes(x = lon, y = lat),
      shape = 8,  # star shape
      size = 0.85,   # adjust to match legend
      color = "black",
      stroke = 0.1,
      inherit.aes = FALSE,
      show.legend = FALSE
...
# Save
ggsave(
  "F3_Terra_Precipitation_Trend.png",
  plot  = final_plot,
  dpi   = 300,
  width = 16, height = 10,
  bg    = "white"
...
# Cities as sf points
spatial_pts <- st_as_sf(era_tp, coords = c("lon", "lat"), crs = 4326)
...
# Save
ggsave(
  "F3_Era_Precipitation_Trend.png",
  plot  = final_plot,
  dpi   = 300,
  width = 16, height = 10,
  bg    = "white"
...
# Cities as sf points
spatial_pts <- st_as_sf(mswep_tp, coords = c("lon", "lat"), crs = 4326)
...
# Save
ggsave(
  "F3_mswep_Precipitation_Trend.png",
  plot  = final_plot,
  dpi   = 300,
  width = 16, height = 10,
  bg    = "white"
...
summary(mswep_tp$Trend)
summary(mswx_tp$Trend)
